...
const createEmptyEmailForm = (): EmailFormState => ({
  name: "Untitled Email",
  subject: "",
  preview_text: "",
  body: "This is a visual layout. Customize it using the editor.",
  type: "Initial",
  category: "email",
  template_format: "visual",
  html_body: null,
...
const toEmailFormState = (template: EmailTemplateRow): EmailFormState => {
...
const EmailBuilder = () => {
  ...
  useEffect(() => {
    if (isNew) {
      let initialForm = createEmptyEmailForm();
      if (presetId) {
        const preset = visualTemplatePresets.find((p) => p.id === presetId && p.category === "email");
        if (preset) {
      ...
      if (found) {
        ...
        setForm(toEmailFormState(found));
        const next = new URLSearchParams(searchParams);
        next.delete("edit");
        setSearchParams(next, { replace: true });
      }
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: ({ template, isNext, wasNew }) => {
      ...
      toast.success(wasNew ? "Template created!" : "Template saved!");
      
      if (isNext) {
        navigate(`/campaigns/new?templateId=${template.id}`);
      } else if (wasNew) {
        navigate(`/emails/builder?edit=${template.id}`, { replace: true });
      }
    },
    ...
    <div className="flex h-screen w-full flex-col bg-muted/10 font-sans">
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-background px-6 shadow-sm z-10">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" className="rounded-full hover:bg-muted" onClick={() => navigate("/emails/templates")}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div className="flex flex-col">
            <span className="text-sm font-semibold text-foreground">Email Builder</span>
            <span className="text-xs text-muted-foreground truncate max-w-[250px]">{form.name}</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => navigate("/emails/templates")}>
            Cancel
          </Button>
          <Button 
            variant="secondary" 
